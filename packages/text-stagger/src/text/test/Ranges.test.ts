...
class TestRanges extends Ranges<Box<TestRanges>> {
	#boxes?: Box<TestRanges>[];

	get boxes() {
		return (this.#boxes ??= this.rects.map(
			(rect): Box<TestRanges> => new Box(this, rect),
		));
	}
}
...
describe("Ranges", () => {
	...
	it("should handle trimming computed ranges", () => {
		const ranges = createTestRanges(["Hello World"]);
		const trimmedRanges = ranges.trimComputedRanges(0, 5);
