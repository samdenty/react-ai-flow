...
import { isTextSplitOffset, Text, TextSplitElement } from "../text/index.js";
...
export interface ElementOptions {
  ...
  gradientWidth?: string | number | ((box: AnimatedBox) => string | number);
  duration?: number;
  delay?: number;
}
...
export class StaggerElement extends Ranges<AnimatedBox> {
  #boxes?: AnimatedBox[];
  ...
  get boxes() {
    return (this.#boxes ??= this.rects.map(
      (rect) => new AnimatedBox(this, rect, this.relativeTo, this.options)
    ));
  }
